...
#             res[i,j] <- res[i,j]+search2(t1-D[from1,v[i]]-1,t2-D[from2,v3[j]]-1,
...
#             res[i,j] <- res[i,j]+search2(t1-D[from1,v[i]]-1,t2,
#                                          v[i],from2,v[-i])
#           }else if(length(v[-i])){
#             res[i,j] <- res[i,j]+search(t1-D[from1,v[i]]-1,v[i],v[-i])
#           }else if(length(v3)){
...
#Solution found in 2023!
v <- input$v[input$f!=0]
cn <- t(combn(v,8))
ecn <- t(apply(cn,1,\(x)v[!v%in%x]))
res <- c()
for(i in 1:nrow(cn)){
  res[i] <- search(26,"AA",cn[i,])+search(26,"AA",ecn[i,])
  cat(i,"\r")
}
max(res)

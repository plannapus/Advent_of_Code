...
for(i in 1:nrow(input)){
  ...
  while(j<=ncol(input)){
    if(input[i,j]%in%0:9){
      ...
      m <- nchar(n)
      if(any(!input[pmax(i-1,1):pmin(i+1,nrow(input)),pmax(j-1,1):pmin(j+m,ncol(input))]%in%c(".",0:9))){
        N <- c(N,as.integer(n))
...
for(g in 1:nrow(gears)){
  ...
  for(k in seq_along(places)){
    if(any(places[[k]][,1]%in%pmax(gears[g,1]-1,1):pmin(gears[g,1]+1,nrow(input))&
           places[[k]][,2]%in%pmax(gears[g,2]-1,1):pmin(gears[g,2]+1,ncol(input)))){
      n <- n+1
      P <- c(P,N[k])
    }
    cat(g,"/",k,"\r")

...
while(length(queue)){ #dfs
  ...
  for(i in rev(seq_along(dd))){
    x <- pos$x+p[i,1]
    y <- pos$y+p[i,2]
    if(any(pos$pathp[,1]==x&pos$pathp[,2]==y)) next #if already passed here, ignore
    v <- map[x,y] #value at position
    if(v=="#") next #no walls
    ...
    if(v=="E"){ #reached the end
      ...
      S <- memo[memo[,1]==x&memo[,2]==y,3] #check the memoization, if higher score than last time, noneed to pursue
      if(s>S){
        next
      }else{
...
while(length(queue)){ #ok this one takes way too long
  l <- length(queue)
  pos <- queue[[l]]
  d <- substr(pos$pathd,nchar(pos$pathd),nchar(pos$pathd))
  nd <- switch(d,"E"="W","S"="N","W"="E","N"="S") #Direction to ignore
  ...
  for(i in rev(seq_along(dd))){
    ...
    if(v=="E"){
      ...
      path <- paste(pos$pathd,dd[i],sep="")
      S <- memo[memo[,1]==x&memo[,2]==y&memo[,3]==dd[i],4] #is it necessary`i don t know`
      if(s>S){
        next
      }else{
